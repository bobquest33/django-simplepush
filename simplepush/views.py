...
from .forms import SimplePushForm, SubscriptionForm
...
def save_info(request):
    ...
    # pass the data through SimplePushForm for validation purpose
    simple_push_form = SimplePushForm(post_data)

    # Check if subscriptioninfo and the web push info both are valid
    if subscription_form.is_valid() and simple_push_form.is_valid():
        # Get the cleaned data in order to get status_type and group_name
        web_push_data = simple_push_form.cleaned_data
        ...
        if request.user.is_authenticated():
            ...
            simple_push_form.save_or_delete(subscription=subscription, 
                user=request.user,status_type=status_type)
